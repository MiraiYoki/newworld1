...
(function(global) {
  ...
  const Favorites = {
    ...
    /** 添加高亮 */
    addHighlight(text, charId, chapterId, sectionId, startOffset, endOffset, extra) {
      const item = {
        id: uid(),
        text,
        charId,
      ...
      if (extra && extra.isCrossBlock) {
        item.isCrossBlock = true;
        item.startSectionId = extra.startSectionId || sectionId;
        item.endSectionId = extra.endSectionId || sectionId;
        if (extra.blockHighlights) {
          item.blockHighlights = extra.blockHighlights;
        }
      }
